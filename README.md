...
- 在作业详情页导出 PDF、HTML、Markdown、TOML，或复制题目文本。
...
- 支持手动勾选和拖拽排序将要导出的篇目，可按时间或名称正序/倒序自动排序。
- 支持按题型筛选导出，图片可选择保留在线链接或转为 base64。
- 悬浮窗支持一键收起，减少遮挡页面内容。
...
- 格式下拉：选择 `HTML` / `Markdown` / `TOML` 后点击 `导出` 下载对应文件。
...
- 格式下拉：选择 `HTML` / `Markdown` / `TOML` 后点击 `导出`，按当前筛选和勾选列表导出 ZIP。
- 齿轮按钮：展开导出设置。
- 篇目列表：手动勾选或取消某个作业，可直接拖拽调整导出顺序。
- `全选` / `全不选`：快速调整篇目列表。
